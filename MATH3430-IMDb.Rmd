...
Alternative to the above graph, using geom_hex to show the density a bit better to help with the over plotting issue.
```{r}
movies_and_ratings %>%
  mutate(runtime_cap = pmin(runtimeMinutes, 300)) %>% 
  ...
  labs(
    title = "Rating vs Year — Hexbins Colored by Average Runtime",
    x = "Year",
    y = "Rating",
    fill = "Avg Runtime"
  )
